...
class AdminUserInfoSchema(ma.SQLAlchemyAutoSchema):
    ...
    class Meta:
        model = User
        fields = ('username', 'created_at', 'ip', 'name', 'email',
                  'phone', 'tariff', 'state', 'balance', '_links')
...
class FullUserInfoSchema(ma.SQLAlchemyAutoSchema):
    ...
    class Meta:
        model = User
        fields = ('username', 'created_at', 'ip', 'name', 'email',
                  'phone', 'tariff', 'state', 'balance', 'address', '_links')
        load_only = ('password',)
...
class RegisterSchema(Schema):
    ...
    tariff = fields.Str(required=True, validate=validate.OneOf([t.value['tariff_name'] for t in Tariffs]))
